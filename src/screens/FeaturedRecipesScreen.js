...
export default function FeaturedRecipesScreen({ navigation }) {
  const {
    colors,
    translate,
    recipes,
  ...
  const [selected, setSelected] = useState(new Set(featuredRecipeIds));
    ...
    setSelected(prev => {
      ...
      if (next.has(id)) {
        ...
        if (next.size >= MAX_FEATURED) {
          Alert.alert('', translate('selectRecipesForFeatured'));
          return prev;
        }
        next.add(id);
      }
  ...
  const handleSave = async () => {
    setSaving(true);
    const result = await saveFeaturedRecipes(Array.from(selected));
    setSaving(false);
    if (result.success) {
  ...
  const renderItem = useCallback(({ item }) => {
    // Override edilmiş statik tariflerde item.id kendi Firebase ID'si — kanonik
    // kimlik overridesStaticId'dir (bkz. DailyMenuScreen.js'teki aynı gerekçe).
